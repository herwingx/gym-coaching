...
export function RoutineCardsClient({ routines: initialRoutines }: RoutineCardsClientProps) {
    ...
    <>
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {routines.map((routine) => {
          ...
          const trainingDaysFromConfig = configuredDays.filter((day) => !day.is_rest_day).length
          ...
          const volumeLabel =
            daysPerWeek >= 6 ? 'Alta frecuencia' : daysPerWeek >= 4 ? 'Frecuencia media' : 'Frecuencia baja'

          const menuSections: AdminCardMenuSection[] = [
            {
              items: [
                { label: 'Ver', icon: <Eye className="mr-2 size-4" />, href: `/admin/routines/${routine.id}` },
                { label: 'Editar', icon: <Edit2 className="mr-2 size-4" />, href: `/admin/routines/${routine.id}/edit` },
              ],
            },
            {
          ...
          return (
            <AdminCardWithActions key={routine.id} menuSections={menuSections} cardClassName="border-muted/70">
              <AdminCardHeaderWithActions menuSections={menuSections}>
                <div className="flex flex-col gap-2">
                  <h3 className="text-lg font-semibold leading-none">{routine.name}</h3>
                  <div className="flex flex-wrap items-center gap-2">
                    <Badge variant="secondary">{volumeLabel}</Badge>
                    <Badge variant="outline">{routine.goal ? getGoalLabel(routine.goal) : 'Objetivo libre'}</Badge>
                  </div>
                </div>
              </AdminCardHeaderWithActions>
              <CardContent className="p-4 pt-3">
                ...
                <div className="flex flex-col gap-2 rounded-lg border border-dashed p-3">
                  ...
                  <div className="overflow-hidden rounded-full border bg-muted">
                    <div className="h-2 bg-primary/80" style={{ width: `${Math.max(intensity, 8)}%` }} />
                  </div>
      ...
      <AlertDialog open={!!deleteId} onOpenChange={(o) => !o && setDeleteId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Se eliminará &quot;{deleteName}&quot; y todos sus días y ejercicios. Esta acción no se
