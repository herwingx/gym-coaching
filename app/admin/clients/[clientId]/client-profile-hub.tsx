...
import { ArrowUpRight, FileImage, History, Dumbbell, Ruler, Notebook, Plus } from 'lucide-react'
...
function trendFromSessions(sessions: any[]) {
...
export function ClientProfileHub({
  ...
  async function saveNotes() {
    try {
      ...
      setIsSaving(false)
      setSaveError(e?.message || 'Error al guardar notas')
    }
  }
            ...
            <CardContent>
              <div className="flex items-start gap-4">
                <Avatar className="h-12 w-12 rounded-xl">
                  {client?.avatar_url ? (
                    <AvatarImage src={client.avatar_url} alt={client.full_name} className="object-cover" />
                  ) : null}
            ...
            <CardContent>
              {routine ? (
                <div className="space-y-2">
                  ...
                  {routine.description ? <div className="text-sm text-muted-foreground">{routine.description}</div> : null}
                  <div className="text-sm text-muted-foreground">
                    Duración: {routine.duration_weeks ? `${routine.duration_weeks} semanas` : '-'} · Días: {routine.days_per_week || routine.routine_days?.length || '-'}
                  </div>

                  <div className="mt-3 space-y-2">
                    {(routine.routine_days || []).slice(0, 7).map((d: any) => (
                      <div key={d.id} className="flex items-center justify-between gap-3 rounded-lg border px-3 py-2">
                        <div className="text-sm font-medium">
                          Día {d.day_number || ''} · {d.day_name || d.day_name}
                        </div>
                        <div className="text-xs text-muted-foreground">
                          {d.routine_exercises?.length || d.routine_exercises?.length || 0} ejercicios
                        </div>
                      </div>
                    ))}
                  </div>
                  ...
                  <div key={s.id} className="flex items-center justify-between gap-3 rounded-lg border px-3 py-2">
                    <div className="min-w-0">
                      <div className="text-sm font-medium truncate">
                        {formatDate(s.started_at)} · {s.routine_days?.day_name ? `(${s.routine_days.day_name})` : ''}
                      </div>
                      <div className="text-xs text-muted-foreground">
          ...
          <Card>
            ...
            <CardContent>
              <div className="space-y-3">
                {(workoutSessions || []).slice(0, 10).map((s: any) => (
                ...
                {(progressPhotos || []).map((p: any) => (
                  <div key={p.id} className="rounded-lg border overflow-hidden">
                    ...
                    <div className="relative aspect-4/3 bg-muted">
                      {p.photo_url ? (
                        <Image src={p.photo_url} alt={`Foto ${p.view_type}`} fill className="object-cover" sizes="(max-width: 768px) 100vw, 33vw" />
                      ) : null}
                    </div>
                  </div>
                ))}
