...
import { deactivateInvitationCode, deleteInvitationCode } from '@/app/actions/invitations'
...
export function InvitationCardsClient({ codes: initialCodes }: InvitationCardsClientProps) {
  ...
  const handleCopy = (code: string) => {
    ...
    navigator.clipboard
      .writeText(url)
      .then(() => {
        toast.success('Enlace copiado. Compártelo por WhatsApp o como prefieras.')
      })
      .catch(() => {
  ...
  const handleDeactivate = async (codeId: string) => {
    ...
    if (result && 'success' in result && result.success) {
      ...
      toast.error(result && 'error' in result ? String(result.error) : 'Error al desactivar')
    }
  }
  ...
  const handleDelete = async () => {
    ...
    if (result && 'success' in result && result.success) {
      ...
      toast.error(result && 'error' in result ? String(result.error) : 'Error al eliminar')
    }
    setIsDeleting(false)
  }
  ...
  return (
    <>
      <div className="grid grid-cols-[repeat(auto-fill,minmax(260px,1fr))] gap-4">
        ...
        if (canDeactivate) {
          menuItems.push(
            { label: 'Copiar', icon: <Copy className="mr-2 size-4" />, onClick: () => handleCopy(code.code) },
            { label: 'Desactivar', icon: <Power className="mr-2 size-4" />, onClick: () => handleDeactivate(code.id) },
          )
        }
          if (canDelete) {
            menuItems.push({
        ...
        const menuSections: AdminCardMenuSection[] = menuItems.length > 0 ? [{ items: menuItems }] : []
        ...
        if (hasMenu) {
          return (
            <AdminCardWithActions key={code.id} menuSections={menuSections} cardClassName={cardClassName}>
              <AdminCardHeaderWithActions menuSections={menuSections}>
                <div className="flex flex-col gap-2">
                  <code className="text-lg font-mono font-bold text-primary block truncate">{code.code}</code>
              ...
              <CardContent className="flex flex-col gap-2 p-4 pt-3">
                ...
                <p className="text-xs text-muted-foreground">
                  Expira: {code.expires_at ? format(new Date(code.expires_at), "d MMM yyyy", { locale: es }) : 'Nunca'}
                </p>
              </CardContent>
            </AdminCardWithActions>
          )
          ...
          <Card key={code.id} className={cardClassName}>
            <CardHeader className="p-4 pb-0">
              <div className="flex flex-col gap-2">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  {isUsed && (
                    <Badge variant="outline" className="bg-success/15 text-success border-success/30">Usado</Badge>
                  )}
                  {isExpired && !isUsed && (
                    <Badge variant="outline" className="bg-destructive/15 text-destructive border-destructive/30">Expirado</Badge>
                  )}
                  {isInactive && (
                    <Badge variant="outline" className="bg-muted-foreground/20 text-muted-foreground">Desactivado</Badge>
                  )}
                  {code.for_role === 'admin' ? (
                    <Badge variant="outline" className="text-amber-600 border-amber-500/50">Coach</Badge>
                  ) : (
