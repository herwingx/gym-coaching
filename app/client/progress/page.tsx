...
export default async function ClientProgressPage() {
  ...
  const [{ data: completedSessions }, { data: prRows }, { data: exerciseLogRows }] =
    await Promise.all([
      supabase
  ...
  const strengthEntries = exercises.map((ex) => ({
    ...
    points: (pointsByExerciseId[ex.id] || []).map((p) => ({
      achieved_at: typeof p.achieved_at === 'string' ? p.achieved_at : new Date(p.achieved_at).toISOString(),
          ...
          <Card className="overflow-hidden border-border/80 shadow-sm ring-1 ring-primary/5">
            ...
            <CardContent className="flex flex-col gap-3">
              {strengthInsight.status === 'ok' ? (
                <div className="rounded-xl border border-border/60 bg-muted/15 p-4 sm:p-5">
                  ...
                  <p className="mt-2 font-semibold leading-snug">
                    {strengthInsight.exerciseName}: +<span className="tabular-nums">{strengthInsight.deltaPct.toFixed(1)}</span>%
              ...
              {volumeInsight.status === 'ok' ? (
                <div className="rounded-xl border border-border/60 bg-muted/15 p-4 sm:p-5">
                  <p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Tendencia de volumen</p>
                  <p className="mt-2 font-semibold leading-snug">
                ...
                <div className="rounded-xl border border-border/60 bg-muted/15 px-4 py-6 text-center sm:text-left">
                  <p className="text-sm text-muted-foreground text-pretty leading-relaxed">
                    {volumeInsight.sessionsWithVolume > 0 ? (
                      <>
                        Tienes <span className="tabular-nums font-medium text-foreground">{volumeInsight.sessionsWithVolume}</span>{' '}
                        sesiones con volumen registrado. Necesitamos al menos{' '}
                        <span className="tabular-nums font-medium text-foreground">{volumeInsight.needed}</span> para estimar una
                        tendencia fiable.
                      </>
                    ) : (
                      <>Registra entrenos completados con volumen para ver la tendencia.</>
                    )}
                  </p>
                </div>
              )}
              ...
              <CardHeader className="pb-3">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <CardDescription>
                      Mejor serie por entreno completado; los hitos de PR se fusionan en la misma línea temporal
                    </CardDescription>
                  </div>
                </div>
              </CardHeader>
