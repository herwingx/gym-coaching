...
export async function useInvitationCode(code: string, userId: string, role?: 'admin' | 'client') {
  ...
  if (atomicError || !atomicResult || !atomicResult.success) {
    return { error: atomicResult?.error || atomicError?.message || 'Error al procesar el código' }
  }

  const { client_id: clientId, created_by: coachId, for_role: forRole } = atomicResult
