...
export async function validateAdminSetupCode(code: string): Promise<{ valid: boolean; error?: string }> {
  ...
  if (!setupCode) {
    return { valid: false, error: 'Configuración incompleta. Contacta al soporte.' }
  }
...
export async function createAdminAccount(formData: FormData) {
  ...
  if (!coachName || coachName.length < 2) {
    return { success: false, error: 'Tu nombre debe tener al menos 2 caracteres' }
  }
  ...
  if (data.user?.identities?.length === 0) {
    ...
    if (signInError) {
      return { success: false, error: 'Este email ya esta registrado. Contraseña incorrecta.' }
    }
